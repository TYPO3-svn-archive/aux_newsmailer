		...
		if (($this->id && $access) || ($BE_USER->user['admin'] && !$this->id))	{
			...
			if (t3lib_div::_GP('cmd')=='previewhtml'){
				...
				if (t3lib_div::_GP('cmd')=='invoke'){
					$this->content.=$LANG->getLL('send').' '.$this->sendMail(t3lib_div::_GP('msg'),t3lib_div::_GP('ctrl')).' '.$LANG->getLL('emails');
				}
	  	...
	  	if ($ctrl['uid']!=0)
	  	{
			...
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
		  	...
		  	if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
				...
				$urlinvoke='index.php?id='.$this->id.'&cmd=invoke&msg=0&ctrl='.$ctrl['uid'];
